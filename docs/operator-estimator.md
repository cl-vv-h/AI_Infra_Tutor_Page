...
入口：性能板块的“单算子理论校核”及 `#/operators/estimate`。`#/operators` 现为真实采样分析工作台，见 [工作台说明](./profiling-workbench.md)。纯客户端，无 API、无模型密钥，不上传或持久化输入与实测值。主动导出 JSON 只含估算参数和结果，不含实测时间及任何 profile 文件。
