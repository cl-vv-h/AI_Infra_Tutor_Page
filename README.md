...
- `#/models/knowledge`：模型知识体系，按“模型结构 → 执行与存储 → 服务与性能”组织 9 个专题、27 个核心概念，连接课程与模型实验。
...
六个技术主题附有研读指南，将公开文章与课程、交互模型图和容量对比连接。分析维度仅是背景索引，不是自动生成的文章结论或事实核查。SGLang / FlashInfer 原站深读入口与自动收录区分展示；FlashInfer 的官方 Atom 已加入采集，但超出 90 天窗口的旧文章不会伪装成新内容。主题指南集中维护在 `src/data/news-study-guides.ts`。
...
每条线索提供概念定义与背景课；六类概念还能直接定位到示例模型的层和模块，KV 示例带入显存预算。教学示例不表示原文采用该模型，不把关键词命中当成全文分析或事实核查。量化线索另有 [vLLM 官方 KV 量化与校准说明](https://docs.vllm.ai/en/stable/features/quantization/quantized_kvcache/)。词表与目的地集中维护在 `src/data/news-learning.ts`；浏览器本地计算，不发送文章、收藏或输入，不使用模型 API，不加载课程正文或模型结构数据来生成推荐。所有五个新闻视图共用这一入口，收藏恢复后按当前词表重新计算。
...
阅读清单支持本机导入本站 v2 JSON：单文件最多 500 条、2 MiB，先预览新增/重复/保留数量，确认后才写入。相同 ID 或规范化原文 URL 视为重复，已有快照不覆盖；任一记录损坏、超长或链接不安全会拒绝整批，不静默跳过。仅保留已知文章字段，移除未知嵌套数据、旧主题标签及原型键；HTTPS 链接不得带凭证、非标准端口或 IP / 本机地址字面量。不会解析 DNS、读取原文或证明网址可信；导入条目明确显示“来源及日期未核验”，不沿用“官方发布时间”徽标。

确认时重新验证数据和当前存储，保留待恢复的旧收藏 ID；与页面状态不一致时拒绝写入，存储额度或权限错误时不更新清单状态。原收藏读取异常或正在迁移时禁用导入。取消、离开页面或改选文件后，过期的异步读取不会恢复旧预览。沿用静态站现有的设备本地收藏，不新增云同步、API 或个人数据提交；清除浏览器数据仍会丢失清单，应自行保管导出文件。跨标签页检查不等于事务锁，导入期间应避免并发编辑，参见 [Web Storage 标准](https://html.spec.whatwg.org/multipage/webstorage.html#introduction-16)。
...
混合精度支持模块默认、具体权重及单层覆盖：可分别选择 Gate/Up/Down、Q/K/V、Router、Norm 等，提供 BF16、FP16、FP32、两种 FP8 scale 粒度、INT8、INT4、MXFP4 与 NVFP4；W4A8 专家默认保留原核对路径。设置贯通逐 rank、全层与 DPA 账本，并通过分享链接恢复。格式含 scale 的计算规则、支持范围与后端限制见[逐权重精度说明](docs/per-weight-precision.md)。

知识体系、模型目录、对比实验共用三级入口。专题页按“定义、关系式、适用范围”呈现；桌面使用侧栏，移动端使用专题选择器。单模型模块知识卡与 V4.1 专题直接显示内容，不使用预测、提问或揭示答案流程。知识配置集中于 `src/data/model-knowledge.ts`，先修关系、课程链接和实验参数由 `npm run test:models:knowledge` 校验。

单模型页面以“结构图 / 缓存容量 / 权重清单”三个工作区组织。默认直接展示交互图；窄屏图解排在模型卡之前，完整层分布按需展开。缓存条件与权重位宽在工作区之间共用，切换不会重置层号、已选模块、B、S、TP、缓存精度或预算。权重账本默认展开；定位权重模块会返回结构图，桌面滚动并聚焦节点，窄屏打开详情，关闭后聚焦所选节点。

工作区通过 Hash URL 的 `view=cache` 或 `view=weights` 分享，默认结构图不额外写参数。旧图解链接仍默认打开结构图，已有 `wbits` / `budget` 等参数保持有效，相关工具在对应工作区中。未知工作区会提示并回退。标签支持左右方向键循环、Home/End 和点击；只有当前标签进入 Tab 序列。非活动面板使用原生 `hidden` 隐藏但保留挂载，以保留未提交预算输入、搜索和折叠状态；它们不是额外模型数据副本，不添加本地持久化或网络请求。结构渲染测试逐模型检查三组标签与面板的关联和可见性；不将静态测试当作浏览器交互测试。
