...
export function kdaMlaCacheParts(cache: KdaMlaCache, sequence: number, batch: number, tp: number, bytes: number) {
  ...
  const kvWidth = cache.latentWidth + (cache.sharedKeyWidth ?? 0)
  const kvBytes = batch * kvLayers * sequence * kvWidth * bytes
  ...
  const growthBytesPerToken = kvLayers * kvWidth * bytes + ((sequence + 1) % cache.indexPool === 0 ? kvLayers * cache.indexWidth * bytes : 0)
