...
export function learningStops(model: ModelArchitecture, layer: number): LearningStop[] {
  ...
  return nodes.map((node) => ({
    node,
    question: node.tone === 'memory' ? '历史信息保存在哪里？S 增长会改变哪些维度？' : node.id.endsWith('-add') ? '参与相加的各路张量来自哪里？为什么 Shape 必须相同？' : node.id === 'moe' ? '路由选择了什么？激活的专家与驻留的专家有什么区别？' : node.id === 'lm-head' ? '何时才进入词表投影？logits 是否已经是采样出的 token？' : node.id === 'embedding' ? '离散 token 如何变成主干向量？Prefill 与 Decode 的 N 有何不同？' : '输入如何变成输出？哪些中间维度或权重随 TP 改变？',
