...
export function learningStops(model: ModelArchitecture, layer: number): LearningStop[] {
  ...
  return nodes.map((node) => node.id === 'hc-expand' ? {
    ...
    node,
    focus: node.id.includes('-res-') ? '深度聚合、冻结 bank 与块内累加' : node.id.startsWith('hc-') ? '四路残差流的投影与写回' : node.tone === 'memory' ? '历史状态与上下文维度' : node.id.endsWith('-add') ? '同形状残差汇合' : node.id.endsWith('moe') ? '专家路由与常驻权重' : node.id === 'lm-head' ? '词表投影与 logits' : node.id === 'embedding' ? 'Token 映射与输入张量' : '张量变换与 TP 分片',
