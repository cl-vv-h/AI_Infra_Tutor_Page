...
export default function WeightPrecisionEditor({ modelId, tp, ep, attentionTp = tp, layer, value, onChange }: {
  ...
  return <details aria-label="逐权重精度" className="mt-5 rounded-xl border border-violet-200/20 bg-black/10 p-4">
    ...
    <p className="mt-3 text-xs leading-6 text-white/60">当前层覆盖 → 同名权重覆盖 → 模块默认。Gate / Up、K / V 及成组 Norm 已分别列出；不同格式不再视为同一个融合容器。专家选择作用于该投影的全部本地专家，不模拟逐专家编号量化。</p>
    ...
    <div className="mt-4 divide-y divide-white/10">{rows.filter(row => row.nodeId === selected).map(row => <div key={row.key} className="grid min-w-0 gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_minmax(180px,0.8fr)]">
