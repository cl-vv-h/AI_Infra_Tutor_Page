...
export default function NewsStudyGuide({ topic, label, count, sources, onLongRead, isLibrary }: { topic: string; label: string; count: number; sources: number; onLongRead: () => void; isLibrary: boolean }) {
  ...
  return <details aria-label={`${label}研读指南`} className="mt-5 rounded-2xl border border-cyan-200/20 bg-gradient-to-br from-cyan-200/5 to-[#0b131c] p-5 lg:px-6">
    ...
    <div><ol className="space-y-3">{guide.points.map((point, index) => <li key={point} className="flex gap-3 text-base leading-7 text-slate-200"><span className="font-mono text-cyan-200/60">0{index + 1}</span><span>{point}</span></li>)}</ol>{guide.links.length > 0 && <div className="mt-5 flex flex-wrap gap-2 border-t border-white/10 pt-4">{guide.links.map((link) => <Link key={link.to} to={link.to} className="rounded-lg border border-white/10 px-3 py-2 text-sm text-cyan-100 hover:bg-white/5">{link.label}<ArrowUpRight className="ml-1 inline h-3.5 w-3.5" /></Link>)}</div>}</div>
    </div>
  </details>
}
