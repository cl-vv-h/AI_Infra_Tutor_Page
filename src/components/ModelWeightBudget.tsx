...
export default function ModelWeightBudget({ model, layer, tp, bits, selectedId, onBits, onSelect }: {
  ...
  return <section aria-label="Decoder 权重账本" className="mt-4 rounded-2xl border border-white/10 bg-[#0b1119] p-4 sm:p-5">
    <h3 className="text-base font-medium text-white"><Layers3 className="mr-2 inline h-4 w-4 text-violet-200" />Decoder 权重账本 <span className="ml-2 text-sm font-normal text-white/60">Layer {layer} · 每卡 {memory(budget.bytes)} · {bits}-bit 理论载荷</span></h3>
