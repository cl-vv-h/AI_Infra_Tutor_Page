...
export default function QwenDenseLayout({ tp, policy, dpaRows, layer = 0 }: { tp: TensorParallelSize; layer?: number; policy: MixedPrecision; dpaRows?: number }) {
  ...
  return <details aria-label="Qwen3 Dense 融合布局" className="mt-4 rounded-xl border border-cyan-200/20 p-4 text-sm leading-6 text-white/70">
    ...
    <p className="mt-3 text-amber-100/80">这是所选存储情景的初始权重布局，不是官方 checkpoint 的原生 dtype 清单。固定版本的 block FP8 要求预先序列化的 FP8 checkpoint 与 dynamic activation；不能只把原 BF16 模型启动参数改成 FP8 就假定得到此布局。不计后端重排/对齐、量化临时张量、KV、Embedding 或 LM Head，也不代表完整显存。默认 BF16 Norm；确定性训练等特殊 FP32 分支不在本情景中。</p>
