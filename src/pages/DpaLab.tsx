  ...
  return <main className="mx-auto max-w-7xl px-5 pb-20 pt-8 text-white">
    ...
    <section className={panel} aria-label="DPA 配置">
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{[
        ['阶段内并行组 / 卡数', 'tp', model.supportedTp], ['Attention DP', 'dp', dpaSizes(state.tp)], ...(!gqa ? [['Expert EP', 'ep', dpaSizes(state.tp)]] : []),
    ...
    <section className={panel} aria-label="DPA padding 与缓存">
      ...
      {gqa ? <div aria-label="GQA DPA 缓存归属" className="mt-3 rounded-xl border border-lime-200/20 p-3 text-sm leading-6 text-white/70"><p className="text-lime-100">KV 总量由请求分组与 KV head 复制共同决定</p><p className="mt-2">当前 rank 的单层逻辑 K+V：[{data.cacheShape!.join(', ')}]，依次为本组请求、历史 S、K/V 两份、本地 KV heads、head_dim；乘 {dimensions.layers} 层及每元素 {state.cacheBytes} B，得到 {group.cacheBytes.toLocaleString('en-US')} B。KV heads {selected.kvHeadsStart}–{selected.kvHeadsEnd} 属于本 Attention TP 分片。</p><p className="mt-2">8 个 KV heads 在 Attention TP≤8 时完整分片，没有 KV head 复制；Attention TP&gt;8 时，每个 KV head 复制 Attention TP/8 份。全卡合计 = 全部请求 × S × 36 × 2 × max(8, Attention TP) × 128 × 每元素字节。当普通 TP 与 Attention TP 均不超过 8 时，全卡 KV 不随 DPA 改变；超过此范围需计入复制倍率。负载不均时每卡容量仍可能不同，通信 padding 不写成历史 KV。</p><p className="mt-2 text-xs text-white/55">逻辑口径不含分页、前缀共享/驻留、scale、KV 容量预分配或检索临时空间。上下文配置上限 40,960；原生 32,768 与质量/扩展配置限制沿用模型说明。</p></div> : <p className="mt-2 text-xs leading-6 text-white/55">沿用 GLM-5.2 的 78 层 × 每历史位置 704 元素（MLA 512+64，Ascend Index K 全层预留 128）逻辑口径；未计分页、前缀驻留、scale、检索临时空间和最大上下文预分配。补齐通信行不写成新的历史 KV。</p>}
    </section>
