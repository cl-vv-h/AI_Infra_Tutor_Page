  ...
  return <main className="mx-auto max-w-7xl px-5 pb-20 pt-8 text-white">
    ...
    <section className={panel} aria-label="V4.1 实验配置">
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="text-sm">独立 DP 副本：{state.replicas}<input aria-label="V4.1 独立副本" type="range" min={0} max={6} value={Math.log2(state.replicas)} onChange={(e) => update({ replicas: 2 ** Number(e.target.value) })} className="mt-4 w-full accent-cyan-200" /></label>
