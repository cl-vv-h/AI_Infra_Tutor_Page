...
import { demoProfile } from '@/lib/profile-demo'
...
export default function ProfilingWorkbench(){
  ...
  const [tab,setTab]=useState<'hotspots'|'compare'>('hotspots'),[selected,setSelected]=useState(''),[sort,setSort]=useState('total'),[page,setPage]=useState(0)
  const [speedup,setSpeedup]=useState('2'),[comparable,setComparable]=useState(false),[exported,setExported]=useState(false)
  const a=baseline.busy?null:baseline.analysis,b=candidate.busy?null:candidate.analysis
  ...
  return <div className="mx-auto max-w-[1440px] px-4 py-8 text-slate-200 sm:px-8 lg:px-12">
    ...
    {a&&<>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><div className="flex gap-2" role="tablist" aria-label="分析视图"><button role="tab" aria-selected={tab==='hotspots'} className={button} onClick={()=>{setTab('hotspots');setPage(0)}}>热点与优化优先级</button><button role="tab" aria-selected={tab==='compare'} className={button} onClick={()=>{setTab('compare');setPage(0)}}>A/B 回归对照</button></div><button className={`${button} flex items-center gap-2`} onClick={download}><Download size={14}/>导出匿名统计</button></div>
      ...
      {pageCount>1&&<div className="mt-4 flex items-center justify-center gap-3 text-xs"><button className={button} disabled={page===0} onClick={()=>setPage(p=>p-1)}>上一页</button><span>{page+1} / {pageCount}</span><button className={button} disabled={page+1>=pageCount} onClick={()=>setPage(p=>p+1)}>下一页</button></div>}
    </>}
