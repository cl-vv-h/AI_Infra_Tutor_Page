...
function qwen35(moe: boolean): ModelArchitecture {
  ...
  const ffn: ArchitectureNode = {
    ...
    weights: moe ? [
      ...
      { name: 'shared_expert.gate_proj / up_proj · each TP local', shape: '[{expertShard}, 2048]' },
      { name: 'shared_expert.down_proj · TP local', shape: '[2048, {expertShard}]' },
      { name: 'shared_expert_gate.weight · replicated', shape: '[1, 2048]' },
    ] : [
      { name: 'gate_proj / up_proj · each TP local', shape: '[{intermediateShard}, 4096]' },
    ...
    nodes: [
      ...
      },
      {
        ...
        weights: [
          ...
          { name: 'in_proj_a / in_proj_b · each TP local', shape: `[{linearValueHeads}, ${hidden}]` },
          { name: 'A_log / dt_bias · each TP local', shape: '[{linearValueHeads}]' },
      ...
      },
      {
        ...
        weights: [
          ...
          { name: 'k_proj / v_proj · each TP local', shape: `[{localKvHeads} × 256, ${hidden}]` },
          { name: 'q_norm / k_norm · each', shape: '[256]', note: '使用 (1 + weight) 缩放；各 TP rank 复制' },
