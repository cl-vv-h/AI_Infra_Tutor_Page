...
try {
  ...
  for (const layer of [0, 11, 12, 13, 84, 91, 92]) {
    ...
    for (const text of ['215.14 MiB', '448 MiB', '512+64=576', '当前前向计算', 'FP32', 'BF16', '跨层残差快照缓冲区（bank）', '块内累加状态（prefix）', '不是硬件存储体', '不指提示词前缀', '预留快照槽位数', '实际张量精度随隐藏状态精度确定']) assert.ok(html.includes(text), text)
    assert.doesNotMatch(html, /深度 bank|冻结 bank|不伪造模型学到的分数/)
  ...
  const residualDecode = renderExplorer('/models/kimi-k3?layer=12&b=4&tp=4&phase=decode')
  assert.ok(residualDecode.includes('448 KiB'))
  assert.ok(residualDecode.includes('PP=1、Attention DP=1、无序列并行'))
  assert.ok(residualDecode.includes('快照槽位 1'))
