...
try {
  ...
  const { availableExpertFormats, compatibleExpertFormats } = await server.ssrLoadModule('/src/lib/expert-packing.ts')
  ...
  for (const model of modelArchitectures) for (const tp of model.supportedTp) for (const ep of expertParallelSizes(model, tp)) {
    ...
    for (const node of decoderNodes(model, layer).filter(n => n.weights.some(w => w.routedExpert))) {
      for (const expertFormat of compatibleExpertFormats(model, tp, ep).length ? compatibleExpertFormats(model, tp, ep) : ['bf16']) {
        ...
        if (availableExpertFormats(model).length) {
          ...
          assert.ok(html.includes('value="' + compatibleExpertFormats(model, tp, ep).indexOf(expertFormat) + '"'))
